    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            function generateDynamicFields() {
                ...
                if (!form1 || !dynamicFieldsContainer) {
                    console.error('Form1 or dynamicFieldsContainer not found');
                    return;
                }

                // Loop through 5 fields
                for (let i = 1; i <= 5; i++) {
                    const inputType = form1.querySelector(`#input_type_${i}`);
                    const inputName = form1.querySelector(`#input_name_${i}`);

                    console.log(`Looking for input_type_${i} and input_name_${i}`); // Logging
                    ...
                    if (inputType && inputName) {
                        ...
                        if (typeValue && nameValue) {
                            ...
                            selectLabel.textContent = 'Select Input Type';
                            const select = document.createElement('select');
                            select.className = 'form-select';
                            select.name = `dynamic_input_type_${i}`;
                            select.innerHTML = `
                        <option value="text"${typeValue === 'text' ? ' selected' : ''}>Text</option>
                        <option value="number"${typeValue === 'number' ? ' selected' : ''}>Number</option>
                        <option value="email"${typeValue === 'email' ? ' selected' : ''}>Email</option>
                    `;
                            selectDiv.appendChild(selectLabel);
                            selectDiv.appendChild(select);
                            col1.appendChild(selectDiv);
                            ...
                            input.name = `dynamic_input_name_${i}`;
                            input.className = 'form-control';
                            input.value = nameValue;
                            inputDiv.appendChild(inputLabel);
                            inputDiv.appendChild(input);
                            col2.appendChild(inputDiv);
                            ...
                            console.warn(
                                `Input type or name for field ${i} not found. TypeValue: ${typeValue}, NameValue: ${nameValue}`
                                );
                        }
                    } else {
                        console.warn(`Input type or name for field ${i} not found`);
                    }
                }
            }
